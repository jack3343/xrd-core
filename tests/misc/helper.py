...
from pyxrdlib.pyxrdlib import XmssFast
...
@contextlib.contextmanager
def set_xrd_dir(data_name):
    dst_dir = tempfile.mkdtemp()
    prev_val = config.user.xrd_dir
    try:
        test_path = os.path.dirname(os.path.abspath(__file__))
        ...
        config.user.xrd_dir = dst_dir
        yield dst_dir
    finally:
        shutil.rmtree(dst_dir)
        config.user.xrd_dir = prev_val
...
def mocked_genesis():
    ...
    with mock.patch('xrd.core.GenesisBlock.GenesisBlock.instance'):
...
def get_token_transaction(xmss1, xmss2, amount1=400000000, amount2=200000000, fee=1) -> TokenTransaction:
    ...
    initial_balances.append(xrd_pb2.AddressAmount(address=xmss1.address,
                                                  amount=amount1))
    initial_balances.append(xrd_pb2.AddressAmount(address=xmss2.address,
                                                  amount=amount2))
...
def gen_blocks(block_count, state, miner_address):
    ...
    with mock.patch('xrd.core.misc.ntp.getTime') as time_mock:
        ...
        for i in range(0, block_count):
            ...
            bm = xrd_pb2.BlockNumberMapping(headerhash=block.headerhash,
