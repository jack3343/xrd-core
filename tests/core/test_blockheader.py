...
@mock.patch('xrd.core.misc.ntp.getTime', return_value=1615270948)
class TestBlockHeader(TestCase):
    def setUp(self):
        with mock.patch('xrd.core.misc.ntp.getTime', return_value=1615270948) as time_mock:
    ...
    def test_validate_fail_headerhash(self, time_mock):
        ...
        with mock.patch('xrd.core.BlockHeader.BlockHeader.generate_headerhash', return_value=b'nonsense'):
            result = self.block_header.validate(self.fee_reward, self.coinbase_amount,
                                                self.block_header.tx_merkle_root, config.dev)
            self.assertFalse(result)
    ...
    def test_validate_fail_rewards_and_wrong_merkleroot(self, time_mock):
        ...
        with mock.patch('xrd.core.BlockHeader.BlockHeader.block_reward_calc', return_value=0):
            result = self.block_header.validate(self.fee_reward, self.coinbase_amount,
                                                self.block_header.tx_merkle_root, config.dev)
            self.assertFalse(result)
