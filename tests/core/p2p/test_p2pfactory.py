...
from xrd.core.Message import Message
from xrd.core.MessageRequest import MessageRequest
...
from xrd.generated import xrd_pb2, xrdlegacy_pb2
...
def make_message(**kwargs):
    return xrdlegacy_pb2.LegacyMessage(**kwargs)
...
@patch('xrd.core.p2p.p2pfactory.logger', autospec=logger, name="Mock Logger")
@patch('xrd.core.p2p.p2pfactory.reactor', autospec=reactor, name="My Mock Reactor")
class TestP2PFactory(TestCase):
    ...
    def test_create_factory(self, m_reactor, m_logger):
        factory = P2PFactory(chain_manager=ChainManager(None), sync_state=None, xrd_node=None)
        self.assertEqual(P2PProtocol, factory.protocol,
                         "Factory has not been assigned the expected protocol")

    @patch('xrd.core.p2p.p2pfactory.config', autospec=True)
    ...
    def test_monitor_connections(self, m_reactor, m_logger):
        ...
        with patch('xrd.core.p2p.p2pfactory.config', autospec=True) as m_config:
    ...
    def test_broadcast(self, m_reactor, m_logger):
        # broadcast msg_type is xrdlegacy_pb2.LegacyMessage
        self.factory.broadcast(xrdlegacy_pb2.LegacyMessage.TX, b'1234')
        self.channel_1.send.assert_called_once()
        self.channel_2.send.assert_called_once()
    ...
    def test_broadcast_does_not_broadcast_to_peers_known_to_have_the_mr(self, m_reactor, m_logger):
        ...
        self.factory.master_mr.requested_hash[b'1234'] = message_request
        self.factory.broadcast(xrdlegacy_pb2.LegacyMessage.SL, b'1234')
        self.channel_1.send.assert_called_once()
...
class TestP2PFactoryCompareAndSync(TestCase):
    def setUp(self):
        ...
        self.node_header_hash = xrd_pb2.NodeHeaderHash(
            block_number=1,
...
class TestP2PFactoryPeerFetchBlock(TestCase):
    def setUp(self):
        ...
        self.m_xrdnode.is_banned.return_value = False
        self.factory.add_connection(self.channel_1)
        self.factory.add_connection(self.channel_2)
        self.factory.add_connection(self.channel_3)
        ...
        self.factory._target_node_header_hash = xrd_pb2.NodeHeaderHash(
            block_number=1,
    ...
    def test_peer_fetch_block_block_not_found_and_retry(self, m_reactor, m_logger):
        ...
        self.m_xrdnode.peer_manager.ban_channel.assert_called_once_with(self.channel_1)
...
class TestP2PFactoryBlockReceived(TestCase):
    def setUp(self):
        ...
        self.factory = P2PFactory(chain_manager=Mock(autospec=ChainManager), sync_state=None, xrd_node=self.m_xrdnode)
        self.factory.pow = Mock(autospec=POW)
