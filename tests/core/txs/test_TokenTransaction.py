...
class TestTokenTransaction(TestCase):
    ...
    def setUp(self):
        self.initial_balances_valid = [xrd_pb2.AddressAmount(address=self.alice.address, amount=1000),
                                       xrd_pb2.AddressAmount(address=self.bob.address, amount=1000)]

        self.params = {"symbol": b'xrd',
    ...
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        # tx = self.make_tx()
        # tx.sign(self.alice)
        # with patch('xrd.core.txs.TokenTransaction.TokenTransaction.initial_balances', new_callable=PropertyMock) as m_i_balances:
