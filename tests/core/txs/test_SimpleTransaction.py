...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, get_slave_xmss, replacement_getTime, set_xrd_dir
...
class TestSimpleTransaction(TestCase):
    ...
    @patch('xrd.core.txs.Transaction.config')
    def test_validate_tx_invalid(self, m_config, m_logger):
        ...
        with patch('xrd.core.txs.TransferTransaction.TransferTransaction.amounts',
        ...
        with patch('xrd.core.txs.TransferTransaction.TransferTransaction.fee', new_callable=PropertyMock) as m_fee:
            m_fee.return_value = -1
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('xrd.core.txs.TransferTransaction.TransferTransaction.addrs_to',
        ...
        with patch('xrd.core.txs.TransferTransaction.TransferTransaction.addrs_to',
            ...
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('xrd.core.txs.TransferTransaction.TransferTransaction.addr_from',
