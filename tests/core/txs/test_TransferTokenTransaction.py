...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, get_slave_xmss, set_xrd_dir

logger.initialize_default()


@patch('xrd.core.txs.Transaction.logger')
class TestTransferTokenTransaction(TestCase):
    ...
    def test_state_validate_tx_custom(self, m_logger):
        ...
        tx = TransferTokenTransaction.create(**params)
        tx.sign(self.alice)
        with patch('xrd.core.txs.Transaction.Transaction.fee', new_callable=PropertyMock) as m_fee:
        ...
        tx = TransferTokenTransaction.create(**params)
        tx.sign(self.alice)
        with patch('xrd.core.txs.TransferTokenTransaction.TransferTokenTransaction.addrs_to',
    ...
    @patch('xrd.core.txs.Transaction.Transaction.validate_slave', return_value=True)
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        tokens.data[(self.alice.address, tx.token_txhash)] = xrd_pb2.TokenBalance(balance=1000,
                                                                                  decimals=0,
                                                                                  delete=False)
        ...
        state_container.tokens.data[(self.alice.address, tx.token_txhash)] = xrd_pb2.TokenBalance(balance=1000,
                                                                                                  decimals=0,
                                                                                                  delete=False)
        ...
        # fee = -1
        with patch('xrd.core.txs.TransferTokenTransaction.TransferTokenTransaction.fee',
        ...
        # total_amount = -1
        with patch('xrd.core.txs.TransferTokenTransaction.TransferTokenTransaction.total_amount',
        ...
        tokens = Indexer(b'token', None)
        tokens.data[(self.alice.address, tx.token_txhash)] = xrd_pb2.TokenBalance(balance=99,
                                                                                  decimals=0,
                                                                                  delete=False)
        state_container.tokens = tokens
        result = tx._validate_extended(state_container)
