...
from xrd.core.ESyncState import ESyncState
from xrd.core.xrdnode import xrdNode
from xrd.core.State import State
from xrd.core.ChainManager import ChainManager
from xrd.core.TransactionPool import TransactionPool
from xrd.core.txs.CoinBase import CoinBase
from xrd.core.txs.TransferTransaction import TransferTransaction
from xrd.core.p2p.p2pprotocol import P2PProtocol
from xrd.core.p2p.p2pPeerManager import P2PPeerManager
from xrd.core.p2p.p2pChainManager import P2PChainManager
from xrd.core.node import POW
...
from tests.misc.helper import set_xrd_dir, get_alice_xmss, get_slave_xmss, replacement_getTime, get_random_xmss
...
class TestxrdNodeReal(TestCase):
    @patch('xrd.core.misc.ntp.getTime', new=replacement_getTime)
    def setUp(self):
    ...
    @patch('xrd.core.xrdnode.xrdNode.block_height', new_callable=PropertyMock, return_value=19)
    def test_get_latest_blocks(self, m_height):
        ...
        latest_blocks = self.xrdnode.get_latest_blocks(0, 1)
        ...
        latest_blocks = self.xrdnode.get_latest_blocks(0, 2)
        ...
        latest_blocks = self.xrdnode.get_latest_blocks(10, 3)
...
class TestxrdNode(TestCase):
    ...
    def setUp(self):
        ...
        # As the xrdNode is instantiated and torn down for each test, the minuscule or negative diff between present
        # time and start_time can cause problems.
        self.xrdnode.start_time -= 10

        self.xrdnode._pow = Mock(autospec=POW)
        self.xrdnode._pow.miner.lock.__enter__ = Mock()
        self.xrdnode._pow.miner.lock.__exit__ = Mock()

        self.xrdnode.peer_manager = Mock(autospec=P2PPeerManager, name='mock P2PPeerManager',
                                         known_peer_addresses=set())
        self.xrdnode.p2pchain_manager = Mock(autospec=P2PChainManager, name='mock P2PChainManager')
    ...
    @patch('xrd.core.BlockMetadata.BlockMetadata.get_block_metadata')
    def test_monitor_chain_state_no_peer_with_higher_difficulty_found(self, mock_get_block_metadata):
        """
        xrdNode.monitor_chain_state() basically:
        ...
        self.xrdnode.peer_manager.get_better_difficulty.return_value = None
        mock_get_block_metadata.return_value = m_block_metadata
        self.chain_manager.last_block.return_value = m_block

        self.xrdnode.monitor_chain_state()

        self.xrdnode.peer_manager.monitor_chain_state.assert_called_once()
        self.xrdnode.peer_manager.get_better_difficulty.assert_called_once()
    ...
    def test_monitor_chain_state_peer_with_higher_difficulty_found(self, mock_get_block_metadata):
        ...
        self.xrdnode.peer_manager.get_better_difficulty.return_value = m_channel
        mock_get_block_metadata.return_value = m_block_metadata
    ...
    def test_start_listening(self):
        ...
        """
        self.assertIsNone(self.xrdnode._p2pfactory)
        self.xrdnode.start_listening()
        self.assertIsNotNone(self.xrdnode._p2pfactory)

    def test_get_address_is_used(self):
        """
        xrdNode.get_address_is_used() asks the DB (State) if a particular address has ever been used.
        It also validates the address before sending it to the State.
        """
        self.db_state.get_address_is_used.return_value = True
        result = self.xrdnode.get_address_is_used(alice.address)
        ...
        with self.assertRaises(ValueError):
            self.xrdnode.get_address_is_used(b'fdsa')

    @patch('xrd.core.PaginatedData.PaginatedData.get_paginated_data')
    @patch('xrd.core.OptimizedAddressState.OptimizedAddressState.get_optimized_address_state')
    @patch('xrd.core.TransactionMetadata.TransactionMetadata.get_tx_metadata')
    def test_get_mini_transactions_by_address(self,
        ...
        response = self.xrdnode.get_mini_transactions_by_address(alice.address, 3, 1)
    ...
    def test_get_optimized_address_state(self, mock_get_optimized_address_state):
        """
        xrdNode.get_address_state() asks the DB (State) for an Address's AddressState, like its nonce, ots index...
        It also validates the address before sending it to the State.
        """
        m_addr_state = Mock(autospec=OptimizedAddressState)
        mock_get_optimized_address_state.return_value = m_addr_state
        result = self.xrdnode.get_optimized_address_state(alice.address)
        self.assertEqual(m_addr_state, result)

        # Fetching AddressState for Coinbase Address
        m_addr_state = Mock(autospec=OptimizedAddressState)
        mock_get_optimized_address_state.return_value = m_addr_state
        result = self.xrdnode.get_optimized_address_state(config.dev.coinbase_address)
        self.assertEqual(m_addr_state, result)

        with self.assertRaises(ValueError):
            self.xrdnode.get_optimized_address_state(b'fdsa')
    ...
    def test_get_addr_from(self):
        ...
        """
        answer = self.xrdnode.get_addr_from(slave.pk, alice.address)
        self.assertEqual(answer, alice.address)

        answer = self.xrdnode.get_addr_from(slave.pk, None)
    ...
    def test_get_blockidx_from_txhash(self, mock_get_tx_metadata):
        ...
        result = self.xrdnode.get_blockidx_from_txhash(b'a txhash')
    ...
    def test_get_block_to_mine(self, m_get_block_metadata):
        ...
        with mock._patch_object(ChainManager, 'last_block') as m_last_block:
            ...
            self.xrdnode.get_block_to_mine(alice.address)

            m_last_block.__get__.assert_called_once()
            m_get_block_metadata.assert_called_once()
            self.xrdnode._pow.miner.get_block_to_mine.assert_called_once_with(alice.address, self.chain_manager.tx_pool,
                                                                              m_block, 0)

    def test_submit_mined_block(self):
        self.xrdnode.submit_mined_block(b'blob')
        self.xrdnode._pow.miner.submit_mined_block.assert_called_once_with(b'blob')

    def test_get_node_info(self):
        # I guess this test is useful for making sure every part of xrdNode is adequately mocked
        ans = self.xrdnode.get_node_info()
        self.assertIsInstance(ans, xrd_pb2.NodeInfo)

    @patch('xrd.core.LastTransactions.LastTransactions.get_last_txs')
    def test_get_latest_transactions(self, mock_get_last_txs):
        ...
        result = self.xrdnode.get_latest_transactions(0, 1)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].i, 19)

        # Given [0, 1, 2... 19], with offset 2 count 3 should return [15, 16, 17]
        result = self.xrdnode.get_latest_transactions(2, 3)
        self.assertEqual(len(result), 3)
    ...
    def test_get_block_timeseries(self):
        ...
        result = self.xrdnode.get_block_timeseries(5)
        ...
        result = self.xrdnode.get_block_timeseries(3)
        result_converted_from_iterator = [r for r in result]
        self.assertEqual(result_converted_from_iterator, m_blockdps_as_list[2:])

        # If we have a blockheight of 0, return []
        with patch('xrd.core.xrdnode.xrdNode.block_height', new_callable=PropertyMock, return_value=0):
            result = self.xrdnode.get_block_timeseries(3)
            result_converted_from_iterator = [r for r in result]  # []
        ...
        result = self.xrdnode.get_block_timeseries(6)
        result_converted_from_iterator = [r for r in result]
        self.assertEqual(result_converted_from_iterator, m_blockdps_as_list)

    @patch('xrd.core.BlockMetadata.BlockMetadata.get_block_metadata')
    @patch('xrd.core.Block.Block.get_block_by_number')
    @patch('xrd.core.ChainManager.ChainManager.height', new_callable=PropertyMock, return_value=3)
    def test_get_blockheader_and_metadata(self, m_height, m_get_block_by_number, m_get_block_metadata):
        ...
        result_header, result_metadata = self.xrdnode.get_blockheader_and_metadata(0)
        ...
        result_header, result_metadata = self.xrdnode.get_blockheader_and_metadata(1)
        self.assertEqual(result_header, blocks[1].blockheader)
        ...
        result_header, result_metadata = self.xrdnode.get_blockheader_and_metadata(3)  # get block #3
        ...
        result_header, result_metadata = self.xrdnode.get_blockheader_and_metadata(2)
    ...
    def test_submit_send_tx(self):
        ...
        with self.assertRaises(ValueError):
            self.xrdnode.submit_send_tx(None)

        self.chain_manager.tx_pool.is_full_pending_transaction_pool.return_value = True
        m_tx = Mock(name='mock Transaction')
        with self.assertRaises(ValueError):
            self.xrdnode.submit_send_tx(m_tx)

        self.chain_manager.tx_pool.is_full_pending_transaction_pool.return_value = False
        self.xrdnode._p2pfactory = Mock(name='mock P2PFactory')
        self.xrdnode.submit_send_tx(m_tx)
        self.xrdnode._p2pfactory.add_unprocessed_txn.assert_called_once()


@patch('xrd.core.misc.ntp.getTime', new=replacement_getTime)
class TestxrdNodeProperties(TestCase):
    @patch('xrd.core.misc.ntp.getTime', new=replacement_getTime)
    def setUp(self):
        state = Mock()
        ...
        self.xrdnode = xrdNode(mining_address=b'')
        self.xrdnode.set_chain_manager(self.m_chain_manager)
        self.xrdnode.peer_manager = self.m_peer_manager

    def test_state(self):
        # If xrdnode._p2pfactory is None, then this should be this value
        self.assertEqual(self.xrdnode.state, ESyncState.unknown.value)

        # Else, it should be whatever this part of p2pfactory says
        m_p2pfactory = Mock()
        m_p2pfactory.sync_state.state.value = "test"
        self.xrdnode._p2pfactory = m_p2pfactory
        self.assertEqual(self.xrdnode.state, "test")

    def test_num_connections(self):
        # If xrdnode._p2pfactory is None, then this should return 0
        self.assertEqual(self.xrdnode.num_connections, 0)

        # otherwise it should return what p2pfactory's num_connections says
        m_p2pfactory = Mock(num_connections=5)
        self.xrdnode._p2pfactory = m_p2pfactory
        self.assertEqual(self.xrdnode.num_connections, 5)

    def test_epoch(self):
        self.assertEqual(self.xrdnode.epoch, 0)

        self.m_chain_manager._last_block = Mock(block_number=5)
        self.assertEqual(self.xrdnode.epoch, (5 // config.dev.blocks_per_epoch))

        self.m_chain_manager._last_block = Mock(block_number=256)
        self.assertEqual(self.xrdnode.epoch, (256 // config.dev.blocks_per_epoch))
    ...
    def test_coin_supply(self):
        with mock._patch_object(ChainManager, 'total_coin_supply') as m_total_coin_supply:
            ...
            self.assertEqual(100, self.xrdnode.coin_supply)
    ...
    def test_coin_supply_max(self):
        ...
        self.assertEqual(self.xrdnode.coin_supply_max, config.dev.max_coin_supply)

    def test_get_peers_stat(self):
        self.xrdnode.get_peers_stat()
