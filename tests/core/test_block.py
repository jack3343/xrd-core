...
from pyxrdlib.pyxrdlib import sha2_256
...
from xrd.crypto.misc import merkle_tx_hash

from tests.misc.helper import replacement_getTime, set_xrd_dir
...
class TestBlock(TestCase):
    ...
    def test_validate(self):
        ...
        with patch('xrd.core.Block.Block.transactions', new_callable=PropertyMock) as m_transactions:
            m_transactions.return_value = []
            result = self.block.validate(m_chain_manager, OrderedDict())
            self.assertFalse(result)

        # There was a problem with the CoinBase TX
        with patch('xrd.core.txs.CoinBase.CoinBase._validate_extended') as m_validate_extended:
...
@patch('xrd.core.misc.ntp.getTime', new=replacement_getTime)
@patch('xrd.core.txs.CoinBase.CoinBase.apply', name='mock CoinBase.apply()',
       return_value=True)
@patch('xrd.core.txs.TransferTransaction.TransferTransaction.apply',
       name='mock TransferTransaction.apply()', return_value=True)
@patch('xrd.core.txs.Transaction.Transaction.validate_slave',
       name='mock Transaction.validate_slave()', return_value=True)
@patch('xrd.core.txs.TransferTransaction.TransferTransaction.validate', name='mock Transfer.validate()',
       return_value=True)
class TestBlockApplyStateChanges(TestCase):
    ...
    def test_last_block(self,
        ...
        with patch("xrd.core.Block.Block.get_block_by_number") as mock_get_block_by_number:
