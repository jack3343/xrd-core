...
from tests.misc.helper import get_alice_xmss, get_slave_xmss, set_xrd_dir
...
class TestOptimizedAddressState(TestCase):
    ...
    @patch('xrd.core.config.DevConfig.ots_tracking_per_page', new_callable=PropertyMock, return_value=1024)
    @patch('xrd.core.config.DevConfig.ots_bitfield_size', new_callable=PropertyMock)
