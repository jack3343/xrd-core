...
from tests.misc.helper import replacement_getTime, set_xrd_dir, get_alice_xmss, get_bob_xmss
...
class TestTransactionPool(TestCase):
    ...
    @patch('xrd.core.TransactionPool.TransactionPool.is_full_transaction_pool', autospec=True)
    ...
    @patch('xrd.core.TransactionPool.config', autospec=True)
    ...
    def test_get_tx_index_from_pool(self):
        ...
        tx3 = make_tx(txhash=b'xrdpink')

        self.txpool.add_tx_to_pool(tx1, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx2, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx3, 1, replacement_getTime())

        idx = self.txpool.get_tx_index_from_pool(b'xrdpink')
    ...
    @patch('xrd.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('xrd.core.TransactionPool.logger')
    @patch('xrd.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('xrd.core.TransactionPool.logger')
    @patch('xrd.core.txs.Transaction.Transaction.from_pbdata', return_value=make_tx())
    @patch('xrd.core.TransactionPool.TransactionPool.add_tx_to_pool', return_value=True)
    ...
    @patch('xrd.core.txs.Transaction.Transaction.from_pbdata', new=replacement_from_pbdata)
    ...
    @patch('xrd.core.TransactionInfo.config', autospec=True)
    @patch('xrd.core.TransactionPool.TransactionPool.is_full_transaction_pool', return_value=False)
