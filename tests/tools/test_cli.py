...
from xrd.cli import xrd as xrd_cli
...
@mock.patch('xrd.cli.grpc.insecure_channel', new=mock.MagicMock())
class TestCLI_Wallet_Gen(TestCase):
    ...
    def wallet_gen_default_height(self):
        result = self.runner.invoke(xrd_cli, ["wallet_gen"])
        wallet = open_wallet()
...
class TestCLI(TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        os.chdir(self.temp_dir)
        self.runner.invoke(xrd_cli, ["wallet_gen", "--height=4"])
    ...
    def test_wallet_ls(self):
        result = self.runner.invoke(xrd_cli, ["wallet_ls"])
        wallet = open_wallet()
    ...
    def test_wallet_ls_verbose(self):
        result = self.runner.invoke(xrd_cli, ["-v", "wallet_ls"])
        wallet = open_wallet()
    ...
    def test_wallet_ls_json(self):
        result = self.runner.invoke(xrd_cli, ["--json", "wallet_ls"])
        wallet = open_wallet()
        self.assertTrue(json.loads(result.output))  # Throws an exception if output is not valid JSON
        self.assertIn(wallet["addresses"][0]["address"], result.output)
        self.assertIn(self.temp_dir, result.output)  # You should know which wallet you've opened.
    ...
    def test_wallet_add(self):
        result = self.runner.invoke(xrd_cli, ["wallet_add", "--height=4"])
        wallet = open_wallet()
    ...
    def test_wallet_add_inherit_encryption_status(self):
        self.runner.invoke(xrd_cli, ["wallet_encrypt"], input='password\npassword\n')
        self.runner.invoke(xrd_cli, ["wallet_add", "--height=4"], input='password\n')
        wallet = open_wallet()
        self.assertTrue(wallet["encrypted"])

    def test_wallet_add_different_hash_function(self):
        self.runner.invoke(xrd_cli, ["wallet_add", "--height=4", "--hash_function=shake256"])
        wallet = open_wallet()
    ...
    def test_wallet_recover_hexseed(self):
        ...
        self.runner.invoke(xrd_cli, ["wallet_recover", "--seed-type=hexseed"],
    ...
    def test_wallet_recover_hexseed_invalid_input(self):
        os.rename("wallet.json", "wallet_orig.json")
        wallet_orig = open_wallet("wallet_orig.json")
        result = self.runner.invoke(xrd_cli, ["wallet_recover", "--seed-type=hexseed"],
    ...
    def test_wallet_recover_mnemonic(self):
        os.rename("wallet.json", "wallet_orig.json")
        wallet_orig = open_wallet("wallet_orig.json")
        self.runner.invoke(xrd_cli, ["wallet_recover", "--seed-type=mnemonic"],
    ...
    def test_wallet_recover_mnemonic_invalid_input(self):
        os.rename("wallet.json", "wallet_orig.json")
        wallet_orig = open_wallet("wallet_orig.json")
        result = self.runner.invoke(xrd_cli, ["wallet_recover", "--seed-type=mnemonic"],
    ...
    def test_wallet_secret(self):
        wallet = open_wallet()
        result = self.runner.invoke(xrd_cli, ["wallet_secret", "--wallet-idx=0"])
        self.assertIn(wallet["addresses"][0]["address"], result.output)
    ...
    def test_wallet_secret_encrypted(self):
        wallet = open_wallet()
        self.runner.invoke(xrd_cli, ["wallet_encrypt"], input='password\npassword\n')
        result = self.runner.invoke(xrd_cli, ["wallet_secret", "--wallet-idx=0"], input='password\npassword\n')
        self.assertIn(wallet["addresses"][0]["address"], result.output)
        self.assertIn(wallet["addresses"][0]["mnemonic"], result.output)
        self.assertIn(wallet["addresses"][0]["hexseed"], result.output)
    ...
    def test_wallet_secret_encrypt_decrypt_wrong(self):
        ...
        result = self.runner.invoke(xrd_cli, ["wallet_decrypt"], input='wrong_password\nwrong_password\n')
        self.assertEquals(1, result.exit_code)
        # TODO: Add appropriate error matching

    def test_wallet_secret_invalid_input(self):
        result = self.runner.invoke(xrd_cli, ["wallet_secret", "--wallet-idx=2"])
        self.assertEqual(result.output.strip(), 'Wallet index not found 2')

    def test_wallet_rm(self):
        self.runner.invoke(xrd_cli, ["wallet_add", "--height=4"])
        wallet = open_wallet()
        result = self.runner.invoke(xrd_cli, ["wallet_rm", "--wallet-idx=1", "--skip-confirmation"], input='y\n')
        self.assertNotIn(wallet["addresses"][1]["address"], result.output)
        result = self.runner.invoke(xrd_cli, ["wallet_rm", "--wallet-idx=0"], input='y\n')
        self.assertIn("No wallet found", result.output)

    def test_wallet_rm_invalid_input(self):
        result = self.runner.invoke(xrd_cli, ["wallet_rm", "--wallet-idx=2", "--skip-confirmation"], input='y\n')
        self.assertEqual(result.output.strip(), 'Wallet index not found 2')

    @mock.patch('xrd.cli.xrd_pb2_grpc.PublicAPIStub', autospec=True)
    def test_state(self, mock_stub):
        ...
        m_node_state_resp = xrd_pb2.GetNodeStateResp()
        m_node_state_resp.ParseFromString(m_node_state_resp_serialized_to_string)

        mock_stub_instance = mock.MagicMock(name='this should be xrd_pb2_grpc.PublicAPIStub(channel)')
        mock_stub_instance.GetNodeState.return_value = m_node_state_resp

        mock_stub.name = 'this should be xrd_pb2_grpc.PublicAPIStub'
        mock_stub.return_value = mock_stub_instance

        result = self.runner.invoke(xrd_cli, ["state"])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_state_json(self, mock_stub):
        ...
        result = self.runner.invoke(xrd_cli, ["--json", "state"])
        print(result.output)
    ...
    @mock.patch('xrd.cli.config', autospec=True)
    @mock.patch('xrd.cli.Transaction', autospec=True)
    @mock.patch('xrd.cli.xrd_pb2_grpc.PublicAPIStub', autospec=True)
    def test_slave_tx_generate(self, mock_stub, mock_transaction, mock_config):
        ...
        # Simplest use case
        result = self.runner.invoke(xrd_cli, [
    ...
    def test_tx_inspect(self):
        result = self.runner.invoke(xrd_cli, ["tx_inspect", "--txblob", bin2hstr_unsigned_tx])
        self.assertTrue(json.loads(result.output))

    def test_tx_inspect_invalid_input(self):
        result = self.runner.invoke(xrd_cli, ["tx_inspect", "--txblob", bin2hstr_unsigned_tx[2:]])
        self.assertNotEqual(result.exit_code, 0)
    ...
    def test_tx_push(self, mock_stub):
        ...
        result = self.runner.invoke(xrd_cli, ["tx_push", "--txblob", bin2hstr_signed_tx])
    ...
    def test_tx_push_invalid_input(self, mock_stub):
        ...
        result = self.runner.invoke(xrd_cli, ["tx_push", "--txblob", bin2hstr_unsigned_tx])
        self.assertEqual(result.exit_code, 1)
        self.assertIn('Signature missing', result.output)

        # It should choke on invalid txblobs
        result = self.runner.invoke(xrd_cli, ["tx_push", "--txblob", bin2hstr_signed_tx[2:]])
        self.assertEqual(result.exit_code, 1)
    ...
    def test_tx_transfer(self, mock_stub):
        ...
        mock_stub.name = 'a fake xrd_pb2_grpc.PublicAPIStub'
        mock_stub.return_value = mock_stub_instance
    ...
    def test_tx_transfer_encrypted_wallet(self, mock_stub):
        ...
        # Simplest use case
        self.runner.invoke(xrd_cli, ["wallet_encrypt"], input='password\npassword\n')
        result = self.runner.invoke(xrd_cli, ["tx_transfer", "--src=0", "--master=", "--dsts={}".format(qaddr_1),
    ...
    def test_tx_token(self, mock_stub):
        ...
        attrs = {"name": "this should be xrd_pb2_grpc.PublicAPIStub(channel)",
                 "PushTransaction.return_value": m_push_transaction_resp}
        mock_stub_instance = mock.MagicMock(**attrs)
    ...
    def test_token_list(self, mock_stub):
        ...
        result = self.runner.invoke(xrd_cli, ["token_list", "--owner={}".format(qaddr_1)])
    ...
    def test_token_list_invalid_input(self, mock_stub):
        ...
        result = self.runner.invoke(xrd_cli, ["token_list", "--owner={}".format(qaddr_1[:-1])])
