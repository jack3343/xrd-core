...
from xrd.core.BlockHeader import BlockHeader
...
from xrd.services.MiningAPIService import MiningAPIService
...
class TestMiningAPI(TestCase):
    ...
    def test_GetBlockMiningCompatible(self):
        ...
        self.xrdnode.get_blockheader_and_metadata = MagicMock(return_value=[block_header, BlockMetadata()])
        ...
        self.xrdnode.get_blockheader_and_metadata = MagicMock(return_value=[None, None])
        answer = self.service.GetBlockMiningCompatible(request=req, context=None)
    ...
    def test_GetBlockToMine(self):
        ...
        difficulty = 100
        self.xrdnode.get_block_to_mine = MagicMock(return_value=(blocktemplate_blob, difficulty))

        req = xrdmining_pb2.GetBlockToMineReq(wallet_address=b'fake address')
        ...
        # What happens if xrdNode could not generate a blocktemplate? Then the response should be blank.
        self.xrdnode.get_block_to_mine.return_value = None
        answer = self.service.GetBlockToMine(request=req, context=None)
    ...
    def test_SubmitMinedBlock(self):
        req = xrdmining_pb2.SubmitMinedBlockReq(blob=b'minedblob')

        # Submit mined block encountered an error
        self.xrdnode.submit_mined_block = MagicMock(return_value=False)
        answer = self.service.SubmitMinedBlock(req, context=None)
        self.assertTrue(answer.error)

        # This time it was successful
        self.xrdnode.submit_mined_block = MagicMock(return_value=True)
        answer = self.service.SubmitMinedBlock(req, context=None)
