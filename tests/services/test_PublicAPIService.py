...
from pyxrdlib.pyxrdlib import str2bin, hstr2bin, bin2hstr
...
from xrd.services.PublicAPIService import PublicAPIService
from tests.blockchain.MockedBlockchain import MockedBlockchain
from tests.misc.helper import get_alice_xmss, get_bob_xmss, replacement_getTime, set_xrd_dir
...
class TestPublicAPI(TestCase):
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            service = PublicAPIService(mock_blockchain.xrdnode)
            request = xrd_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getAddressFromPK(self):
        ...
        request = xrd_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    @patch('xrd.core.config.DevConfig', new_callable=PropertyMock)
    ...
    def test_getBlockByNumber(self):
        with set_xrd_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           xrd_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = xrd_pb2.GetBlockByNumberReq(block_number=b.block_number)
