...
from pyxrdlib.pyxrdlib import bin2hstr, hstr2bin

from xrd.core.misc import logger
from xrd.core.AddressState import AddressState
from xrd.daemon.walletd import WalletD
from xrd.generated import xrdwallet_pb2, xrd_pb2
from xrd.services.WalletAPIService import WalletAPIService
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_xrd_dir, replacement_getTime
...
class TestWalletAPI(TestCase):
    ...
    def test_addAddressFromSeed(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd = WalletD()
            service = WalletAPIService(walletd)
            resp = service.AddAddressFromSeed(xrdwallet_pb2.AddAddressFromSeedReq(seed=hex_seed), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_addAddressFromSeed2(self):
        with set_xrd_dir("wallet_ver1"):
            walletd = WalletD()
            service = WalletAPIService(walletd)
            resp = service.AddAddressFromSeed(xrdwallet_pb2.AddAddressFromSeedReq(seed=self.mnemonic), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_listAddresses(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.ListAddresses(xrdwallet_pb2.ListAddressesReq(), context=None)
    ...
    def test_removeAddress(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.RemoveAddress(xrdwallet_pb2.RemoveAddressReq(address=address), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_isValidAddress(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            qaddress = "Q010400ff39df1ba4d1d5b8753e6d04c51c34b95b01fc3650c10ca7b296a18bdc105412c59d0b3b"
            resp = service.IsValidAddress(xrdwallet_pb2.ValidAddressReq(address=qaddress), context=None)
    ...
    def test_relayTransferTxn(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.RelayTransferTxn(xrdwallet_pb2.RelayTransferTxnReq(addresses_to=qaddresses_to,
                                                                              amounts=amounts,
                                                                              fee=100000000,
                                                                              master_address=None,
    ...
    def test_relayTransferTxnBySlave(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.RelayTransferTxnBySlave(
                xrdwallet_pb2.RelayTransferTxnBySlaveReq(addresses_to=qaddresses_to,
                                                         amounts=amounts,
                                                         fee=100000000,
    ...
    def test_relayMessageTxnBySlave(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.RelayMessageTxnBySlave(
                xrdwallet_pb2.RelayMessageTxnReq(message=b'Hello xrd!',
                                                 fee=100000000,
                                                 master_address=qaddress), context=None)
    ...
    def test_relayTransferTokenTxnBySlave(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.RelayTransferTokenTxnBySlave(
                xrdwallet_pb2.RelayTransferTokenTxnBySlaveReq(addresses_to=qaddresses_to,
                                                              amounts=amounts,
                                                              token_txhash='',
                                                              fee=100000000,
                                                              master_address=qaddress), context=None)
    ...
    def test_encryptWallet(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            service.AddNewAddress(xrdwallet_pb2.AddNewAddressReq(), context=None)

            resp = service.EncryptWallet(xrdwallet_pb2.EncryptWalletReq(), context=None)
            self.assertEqual(resp.code, 1)

            resp = service.EncryptWallet(xrdwallet_pb2.EncryptWalletReq(passphrase=self.passphrase), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_lockWallet(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.UnlockWallet(xrdwallet_pb2.UnlockWalletReq(passphrase=self.passphrase), context=None)
            self.assertEqual(resp.code, 0)

            resp = service.ListAddresses(xrdwallet_pb2.ListAddressesReq(), context=None)
            self.assertEqual(resp.code, 0)

            resp = service.LockWallet(xrdwallet_pb2.LockWalletReq(), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_unlockWallet(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.UnlockWallet(xrdwallet_pb2.UnlockWalletReq(), context=None)
            self.assertEqual(resp.code, 1)

            resp = service.UnlockWallet(xrdwallet_pb2.UnlockWalletReq(passphrase="wrong"), context=None)
            self.assertEqual(resp.code, 1)
    ...
    def test_changePassphrase(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.ChangePassphrase(
                xrdwallet_pb2.ChangePassphraseReq(oldPassphrase=self.passphrase,
            ...
            resp = service.UnlockWallet(xrdwallet_pb2.UnlockWalletReq(passphrase=new_passphrase), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_getTransactionsByAddress(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetMiniTransactionsByAddress = Mock(
                return_value=xrd_pb2.GetMiniTransactionsByAddressResp(mini_transactions=[],
                                                                      balance=0))

            resp = service.GetTransactionsByAddress(
                xrdwallet_pb2.TransactionsByAddressReq(address=get_alice_xmss(4).qaddress), context=None)
    ...
    def test_getTransaction(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetTransaction = Mock(
                return_value=xrd_pb2.GetTransactionResp(tx=tx, confirmations=10))

            resp = service.GetTransaction(xrdwallet_pb2.TransactionReq(tx_hash=tx.transaction_hash), context=None)
    ...
    def test_getBalance(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetBalance = Mock(
                return_value=xrd_pb2.GetBalanceResp(balance=1000))

            resp = service.GetBalance(xrdwallet_pb2.BalanceReq(address=self.qaddress), context=None)
    ...
    def test_getTotalBalance(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetTotalBalance = Mock(
                return_value=xrd_pb2.GetTotalBalanceResp(balance=6000))

            resp = service.GetTotalBalance(xrdwallet_pb2.TotalBalanceReq(), context=None)
    ...
    def test_getOTS(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            ots_bitfield_by_page = xrd_pb2.OTSBitfieldByPage(ots_bitfield=[b'\x00'] * 10,
                                                             page_number=1)
            walletd._public_stub.GetOTS = Mock(
                return_value=xrd_pb2.GetOTSResp(ots_bitfield_by_page=[ots_bitfield_by_page],
                                                next_unused_ots_index=1,
                                                unused_ots_index_found=True))

            resp = service.GetOTS(xrdwallet_pb2.OTSReq(address=self.qaddress), context=None)
            self.assertEqual(resp.code, 0)
    ...
    def test_getHeight(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetHeight = Mock(
                return_value=xrd_pb2.GetHeightResp(height=1001))

            resp = service.GetHeight(xrdwallet_pb2.HeightReq(), context=None)
    ...
    def test_getBlock(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            walletd._public_stub.GetBlock = Mock(
                return_value=xrd_pb2.GetBlockResp(block=block))

            resp = service.GetBlock(xrdwallet_pb2.BlockReq(header_hash=b'001122'), context=None)
    ...
    def test_getBlockByNumber(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.GetBlockByNumber(xrdwallet_pb2.BlockByNumberReq(block_number=1), context=None)
    ...
    def test_getAddressFromPK(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            resp = service.GetAddressFromPK(xrdwallet_pb2.AddressFromPKReq(pk=pk), context=None)
    ...
    def test_getNodeInfo(self):
        with set_xrd_dir("wallet_ver1"):
            ...
            node_info = xrd_pb2.NodeInfo(version=version,
            ...
            walletd._public_stub.GetNodeState = Mock(
                return_value=xrd_pb2.GetNodeStateResp(info=node_info))

            resp = service.GetNodeInfo(xrdwallet_pb2.NodeInfoReq(), context=None)
