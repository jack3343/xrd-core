...
class TestBlockchain(TestCase):
    ...
    def test_sequential_index(self):
        number_blocks = 10
        with MockedBlockchain.create(number_blocks) as mock_blockchain:
            block = mock_blockchain.xrdnode.get_block_last()

            while block is not None:
                prev_block = mock_blockchain.xrdnode.get_block_from_hash(block.prev_headerhash)
                if prev_block:
        ...
        with MockedBlockchain.create(number_blocks) as mock_blockchain:
            ...
            while block is not None:
                ...
                if prev_block:
                    ...
                    block_from_index = mock_blockchain.xrdnode.get_block_from_index(block.block_number)
