...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_xrd_dir


class MockedBlockchain(object):
    MAXNUMBLOCKS = 1000

    def __init__(self, xrdnode, time_mock, ntp_mock):
    ...
    def create_block(self, prev_hash, mining_address=None):
        ...
        dev_config = self.xrdnode._chain_manager.get_config_by_block_number(block_new.block_number)
        while not self.xrdnode._chain_manager.validate_mining_nonce(blockheader=block_new.blockheader,
    ...
    def validate(self, block):
        if not block.validate(self.xrdnode._chain_manager, {}):
    ...
    def add_block(self, block):
        self.validate(block)
        return self.xrdnode._chain_manager.add_block(block)

    def add_new_block(self, mining_address=None):
        block_prev = self.xrdnode.get_block_last()
    ...
    def create(num_blocks, mining_address=None):
        ...
        with mock.patch('xrd.core.misc.ntp.getTime') as ntp_mock, \
                set_xrd_dir('no_data'), \
                State() as state, \
            ...
            try:
                ...
                mock_blockchain = MockedBlockchain(xrdnode, time_mock, ntp_mock)
