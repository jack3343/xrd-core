...
from pyxrdlib.pyxrdlib import bin2hstr  # noqa
from twisted.internet.protocol import Protocol, connectionDone

from xrd.core import config
from xrd.core.OutgoingMessage import OutgoingMessage
from xrd.core.misc import logger, ntp
from xrd.core.p2p.IPMetadata import IPMetadata
from xrd.core.p2p.p2pObservable import P2PObservable
...
class P2PProtocol(Protocol):
    ...
    def peer_manager(self):
        ...
        return self.factory._xrd_node.peer_manager

    @property
    def p2pchain_manager(self):
        # FIXME: this is breaking encapsulation
        return self.factory._xrd_node.p2pchain_manager

    @property
    def tx_manager(self):
        # FIXME: this is breaking encapsulation
        return self.factory._xrd_node.tx_manager
    ...
    def dataReceived(self, data: bytes) -> None:
        ...
        if msg is not None and read_bytes[0] and msg.func_name != xrdlegacy_pb2.LegacyMessage.P2P_ACK:
            p2p_ack = xrd_pb2.P2PAcknowledgement(bytes_processed=read_bytes[0])
            msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.P2P_ACK,
                                              p2pAckData=p2p_ack)
            self.send(msg)
    ...
    def send(self, message: xrdlegacy_pb2.LegacyMessage):
    ...
    def _wrap_message(protobuf_obj) -> Optional[bytes]:
        ...
        '000000191a170a0776657273696f6e120c67656e657369735f68617368'
                ...
                msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.PL,
                                          plData=xrdlegacy_pb2.PLData(peer_ips=trusted_peers,
                                                                      public_port=config.user.p2p_public_port))

        >>> plData = xrdlegacy_pb2.PLData(peer_ips=[])
        >>> msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.PL, plData=plData)
        >>> bin2hstr(P2PProtocol._wrap_message(msg))
        '0000000408012200'
        """
    ...
    def send_version_request(self):
        msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.VE,
                                          veData=xrdlegacy_pb2.VEData())
        self.send(msg)
    ...
    def send_sync(self, synced=False):
        ...
        msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.SYNC,
                                          syncData=xrdlegacy_pb2.SYNCData(state=state_str))
        self.send(msg)
    ...
    def send_fetch_block(self, block_idx):
        ...
        msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.FB,
                                          fbData=xrdlegacy_pb2.FBData(index=block_idx))
        self.send(msg)
    ...
    def send_get_headerhash_list(self, current_block_height):
        ...
        node_header_hash = xrd_pb2.NodeHeaderHash(block_number=start_blocknumber,
                                                  headerhashes=[])
