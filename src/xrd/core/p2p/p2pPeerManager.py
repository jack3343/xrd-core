...
from xrd.core.misc.expiring_set import ExpiringSet
from xrd.core.notification.Observable import Observable
from xrd.core.notification.ObservableEvent import ObservableEvent
...
class P2PPeerManager(P2PBaseObserver):
    ...
    def new_channel(self, channel):
        ...
        self._peer_node_status[channel] = xrd_pb2.NodeChainState(block_number=0,
                                                                 header_hash=b'',
                                                                 cumulative_difficulty=b'\x00' * 32,
                                                                 timestamp=ntp.getTime())
        channel.register(xrdlegacy_pb2.LegacyMessage.VE, self.handle_version)
        channel.register(xrdlegacy_pb2.LegacyMessage.PL, self.handle_peer_list)
        channel.register(xrdlegacy_pb2.LegacyMessage.CHAINSTATE, self.handle_chain_state)
        channel.register(xrdlegacy_pb2.LegacyMessage.SYNC, self.handle_sync)
        channel.register(xrdlegacy_pb2.LegacyMessage.P2P_ACK, self.handle_p2p_acknowledgement)
    ...
    def handle_version(self, source, message: xrdlegacy_pb2.LegacyMessage):
        """
        Version
        ...
        """
        self._validate_message(message, xrdlegacy_pb2.LegacyMessage.VE)

        if not message.veData.version:
            msg = xrdlegacy_pb2.LegacyMessage(
                func_name=xrdlegacy_pb2.LegacyMessage.VE,
                veData=xrdlegacy_pb2.VEData(version=config.dev.version,
    ...
    def handle_peer_list(self, source, message: xrdlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, xrdlegacy_pb2.LegacyMessage.PL)
    ...
    def handle_sync(self, source, message: xrdlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, xrdlegacy_pb2.LegacyMessage.SYNC)
        if message.syncData.state == '':
            if source.factory.synced:
                source.send_sync(synced=True)

    @staticmethod
    def send_node_chain_state(dest_channel, node_chain_state: xrd_pb2.NodeChainState):
        # FIXME: Not sure this belongs to peer management
        msg = xrdlegacy_pb2.LegacyMessage(func_name=xrdlegacy_pb2.LegacyMessage.CHAINSTATE,
    ...
    def broadcast_chain_state(self, node_chain_state: xrd_pb2.NodeChainState):
        # FIXME: Not sure this belongs to peer management
    ...
    def handle_chain_state(self, source, message: xrdlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, xrdlegacy_pb2.LegacyMessage.CHAINSTATE)
    ...
    def handle_p2p_acknowledgement(self, source, message: xrdlegacy_pb2.LegacyMessage):
        P2PBaseObserver._validate_message(message, xrdlegacy_pb2.LegacyMessage.P2P_ACK)
    ...
    def get_peers_stat(self) -> list:
        ...
        for source in list(self.peer_node_status.keys()):
            try:
                peer_stat = xrd_pb2.PeerStat(peer_ip=source.peer.ip.encode(),
