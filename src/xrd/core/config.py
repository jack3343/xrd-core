...
from xrd import __version__ as version
...
class UserConfig(object):
    ...
    def __init__(self, ignore_check=False):
        ...
        self._xrd_dir = expanduser(os.path.join("~/.xrd"))
    ...
    @property
    def xrd_dir(self):
        return self._xrd_dir

    @xrd_dir.setter
    def xrd_dir(self, new_xrd_dir):
        self._xrd_dir = new_xrd_dir
        self.load_yaml(self.config_path)

    @property
    def wallet_dir(self):
        return expanduser(self.xrd_dir)

    @property
    def data_dir(self):
        return expanduser(os.path.join(self.xrd_dir, "data"))

    @property
    def config_path(self):
        return expanduser(os.path.join(self.xrd_dir, "config.yml"))

    @property
    def log_path(self):
        return expanduser(os.path.join(self.xrd_dir, "xrd.log"))

    @property
    def walletd_log_path(self):
        return expanduser(os.path.join(self.xrd_dir, "walletd.log"))

    @property
    def mining_pool_payment_wallet_path(self):
        return expanduser(os.path.join(self.xrd_dir, 'payment_slaves.json'))
...
class DevConfig(object):
    ...
    def __init__(self, pbdata, ignore_check=False, ignore_singleton=False):
        ...
        self.slave_dat_filename = 'slave.xrd'
        self.banned_peers_filename = 'banned_peers.xrd'
    ...
    def create(prev_state_key,
        ...
        chain = xrd_pb2.DevConfig.Chain(reorg_limit=300,
                                        max_coin_supply=105000000,
                                        complete_emission_time_span_in_years=200)
        block_size_controller = xrd_pb2.DevConfig.Block.BlockSizeController(number_of_blocks_analyze=10,
                                                                            size_multiplier=110,  # 1.1
                                                                            block_min_size_limit_in_bytes=1024 * 1024)
        block = xrd_pb2.DevConfig.Block(mining_nonce_offset=39,
                                        extra_nonce_offset=43,
                                        mining_blob_size_in_bytes=76,
                                        block_timing_in_seconds=60,
                                        block_size_controller=block_size_controller)

        message = xrd_pb2.DevConfig.Transaction.Message(max_length=80)

        slave = xrd_pb2.DevConfig.Transaction.Slave(slave_pk_max_length=67)

        token = xrd_pb2.DevConfig.Transaction.Token(symbol_max_length=10,
                                                    name_max_length=30)

        lattice = xrd_pb2.DevConfig.Transaction.Lattice(pk1_max_length=1088,
                                                        pk2_max_length=1472,
                                                        pk3_max_length=65)

        foundation_multi_sig = xrd_pb2.DevConfig.Transaction.FoundationMultiSig(threshold_percentage=10)

        # proposal_config_activation_delay should not be less than reorg_limit
        proposal = xrd_pb2.DevConfig.Transaction.Proposal(threshold_per=51,
        ...
        transaction = xrd_pb2.DevConfig.Transaction(multi_output_limit=100,
                                                    message=message,
                                                    slave=slave,
                                                    token=token,
                                                    lattice=lattice,
        ...
        dev_config = xrd_pb2.DevConfig(chain=chain,
                                       block=block,
                                       transaction=transaction,
                                       pow=xrd_pb2.DevConfig.POW(N_measurement=30, kp=5))
