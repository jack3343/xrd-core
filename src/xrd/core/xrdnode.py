...
from pyxrdlib.pyxrdlib import xrdHelper, bin2hstr
...
from xrd.core.misc import ntp
from xrd.core.misc.logger import logger
from xrd.core.node import POW, SyncState
from xrd.core.p2p.p2pChainManager import P2PChainManager
from xrd.core.p2p.p2pPeerManager import P2PPeerManager
from xrd.core.p2p.p2pTxManagement import P2PTxManagement
from xrd.core.p2p.p2pfactory import P2PFactory
...
class xrdNode:
    ...
    def start_listening(self):
        self._p2pfactory = P2PFactory(chain_manager=self._chain_manager,
                                      ...
                                      xrd_node=self)  # FIXME: Try to avoid cyclic references
    ...
    def get_addr_from(xmss_pk, master_addr):
        ...
        return bytes(xrdHelper.getAddress(xmss_pk))
    ...
    def get_ots(self,
        ...
        for page in range(page_from, max_pages + 1):
            ...
            bitfields.append(xrd_pb2.OTSBitfieldByPage(ots_bitfield=bitfield, page_number=page))
    ...
    def get_mini_transactions_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = xrd_pb2.GetMiniTransactionsByAddressResp()
        for tx_hash in transaction_hashes:
            mini_transaction = xrd_pb2.MiniTransaction()
    ...
    def get_transactions_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        for tx_hash in transaction_hashes:
            ...
            transaction_detail = xrd_pb2.GetTransactionResp(tx=tx.pbdata,
    ...
    def get_multi_sig_spend_txs_by_address(self,
        ...
        response = xrd_pb2.GetMultiSigSpendTxsByAddressResp()
    ...
    def get_vote_stats(self, multi_sig_spend_tx_hash: bytes):
        ...
        return xrd_pb2.GetVoteStatsResp(vote_stats=vote_stats.pbdata)
    ...
    def get_tokens_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = xrd_pb2.GetTokensByAddressResp()
        for tx_hash in token_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            balance = self._chain_manager.get_token(address, tx.txhash)
            transaction_detail = xrd_pb2.TokenDetail(token_txhash=tx.txhash,
                                                     name=tx.name,
                                                     symbol=tx.symbol,
                                                     balance=balance)
    ...
    def get_slaves_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = xrd_pb2.GetSlavesByAddressResp()
        for tx_hash in slave_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            for index in range(0, len(tx.slave_pks)):
                transaction_detail = xrd_pb2.SlaveDetail(slave_address=bytes(xrdHelper.getAddress(tx.slave_pks[index])),
    ...
    def get_lattice_pks_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = xrd_pb2.GetLatticePKsByAddressResp()
        for tx_hash in lattice_pk_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            transaction_detail = xrd_pb2.LatticePKsDetail(pk1=tx.pk1,
                                                          pk2=tx.pk2,
                                                          pk3=tx.pk3,
                                                          tx_hash=tx_hash)
    ...
    def get_multi_sig_addresses_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = xrd_pb2.GetMultiSigAddressesByAddressResp()
        for multi_sig_address in multi_sig_addresses:
            multi_sig_detail = xrd_pb2.MultiSigDetail(
    ...
    def get_node_info(self) -> xrd_pb2.NodeInfo:
        info = xrd_pb2.NodeInfo()
    ...
    def get_block_timeseries(self, block_count) -> Iterator[xrd_pb2.BlockDataPoint]:
        result = []
