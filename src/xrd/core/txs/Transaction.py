...
from pyxrdlib.pyxrdlib import bin2hstr, xrdHelper, XmssFast
...
from xrd.core.txs import build_tx as main_build_tx
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def __init__(self, protobuf_transaction=None):
        ...
        if protobuf_transaction is None:
            self._data = xrd_pb2.Transaction()
    ...
    def addr_from(self):
        ...
        return bytes(xrdHelper.getAddress(self.PK))
    ...
    @staticmethod
    def from_pbdata(pbdata: xrd_pb2.Transaction):
    ...
    @staticmethod
    def get_slave(tx):
        addr_from_pk = bytes(xrdHelper.getAddress(tx.PK))
    ...
    def set_affected_address(self, addresses_set: set):
        ...
        addresses_set.add(bytes(xrdHelper.getAddress(self.PK)))
    ...
    def _coinbase_filter(self):
        if config.dev.coinbase_address in [bytes(xrdHelper.getAddress(self.PK)), self.master_addr]:
