...
from xrd.core import config, BlockHeader
from xrd.core.config import DevConfig
from xrd.core.StateContainer import StateContainer
from xrd.core.StateMigration import StateMigration
from xrd.core.AddressState import AddressState
from xrd.core.VoteStats import VoteStats
from xrd.core.LastTransactions import LastTransactions
...
from xrd.core.PaginatedData import PaginatedData
...
from xrd.generated import xrd_pb2, xrdstateinfo_pb2
...
class ChainManager:
    ...
    def get_address_state(self, address: bytes) -> AddressState:
        ...
        for page in range(0, max_token_page + 1):
            ...
            for token_txn_hash in page_data:
                ...
                # is done by a xrd address as an owner, which has not been
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> xrd_pb2.SlaveMetadata:
        with self.lock:
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> xrd_pb2.TokenBalance:
        with self.lock:
    ...
    def get_headerhashes(self, start_blocknumber):
        with self.lock:
            ...
            node_header_hash = xrd_pb2.NodeHeaderHash()
    ...
    def load(self, genesis_block):
        ...
        if height == -1:
            ...
            block_number_mapping = xrd_pb2.BlockNumberMapping(headerhash=genesis_block.headerhash,
    ...
    def _try_branch_add_block(self, block, dev_config: DevConfig, check_stale=True) -> bool:
        ...
        if new_block_difficulty > last_block_difficulty:
            if self._last_block.headerhash != block.prev_headerhash:
                fork_state = xrdstateinfo_pb2.ForkState(initiator_headerhash=block.headerhash)
    ...
    def _rollback(self, forked_header_hash: bytes, fork_state: xrdstateinfo_pb2.ForkState = None):
        """
    ...
    def add_chain(self, hash_path: list, fork_state: xrdstateinfo_pb2.ForkState) -> bool:
        """
    ...
    def _fork_recovery(self, block: Block, fork_state: xrdstateinfo_pb2.ForkState) -> bool:
    ...
    def _update_block_number_mapping(self, block, batch):
        block_number_mapping = xrd_pb2.BlockNumberMapping(headerhash=block.headerhash,
    ...
    def update_state_container(self,
        ...
        if isinstance(tx, TransferTokenTransaction):
            ...
            if key not in state_container.lattice_pk.data:
                lattice_pk.load(key, xrd_pb2.LatticePKMetadata())
    ...
    def get_block_datapoint(self, headerhash):
        ...
        data_point = xrd_pb2.BlockDataPoint()
