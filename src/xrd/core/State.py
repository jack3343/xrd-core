...
class State:
    ...
    def put_fork_state(self, fork_state: xrdstateinfo_pb2.ForkState, batch=None):
        self._db.put_raw(b'fork_state', fork_state.SerializeToString(), batch)

    def get_fork_state(self) -> Optional[xrdstateinfo_pb2.ForkState]:
        try:
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> xrd_pb2.SlaveMetadata:
        ...
        try:
            slave_metadata = xrd_pb2.SlaveMetadata()
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> xrd_pb2.TokenBalance:
        try:
            token_balance = xrd_pb2.TokenBalance()
