# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: xrdbase.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor.FileDescriptor(
  name='xrdbase.proto',
  package='xrd',
  syntax='proto3',
  serialized_pb=_b('\n\rxrdbase.proto\x12\x03xrd\"\x10\n\x0eGetNodeInfoReq\"5\n\x0fGetNodeInfoResp\x12\x0f\n\x07version\x18\x01 \x01(\t\x12\x11\n\tgrpcProto\x18\x02 \x01(\t2B\n\x04\x42\x61se\x12:\n\x0bGetNodeInfo\x12\x13.xrd.GetNodeInfoReq\x1a\x14.xrd.GetNodeInfoResp\"\x00\x62\x06proto3')
)




_GETNODEINFOREQ = _descriptor.Descriptor(
  name='GetNodeInfoReq',
  full_name='xrd.GetNodeInfoReq',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=22,
  serialized_end=38,
)


_GETNODEINFORESP = _descriptor.Descriptor(
  name='GetNodeInfoResp',
  full_name='xrd.GetNodeInfoResp',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='version', full_name='xrd.GetNodeInfoResp.version', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='grpcProto', full_name='xrd.GetNodeInfoResp.grpcProto', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=40,
  serialized_end=93,
)

DESCRIPTOR.message_types_by_name['GetNodeInfoReq'] = _GETNODEINFOREQ
DESCRIPTOR.message_types_by_name['GetNodeInfoResp'] = _GETNODEINFORESP
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

GetNodeInfoReq = _reflection.GeneratedProtocolMessageType('GetNodeInfoReq', (_message.Message,), dict(
  DESCRIPTOR = _GETNODEINFOREQ,
  __module__ = 'xrdbase_pb2'
  # @@protoc_insertion_point(class_scope:xrd.GetNodeInfoReq)
  ))
_sym_db.RegisterMessage(GetNodeInfoReq)

GetNodeInfoResp = _reflection.GeneratedProtocolMessageType('GetNodeInfoResp', (_message.Message,), dict(
  DESCRIPTOR = _GETNODEINFORESP,
  __module__ = 'xrdbase_pb2'
  # @@protoc_insertion_point(class_scope:xrd.GetNodeInfoResp)
  ))
_sym_db.RegisterMessage(GetNodeInfoResp)



_BASE = _descriptor.ServiceDescriptor(
  name='Base',
  full_name='xrd.Base',
  file=DESCRIPTOR,
  index=0,
  options=None,
  serialized_start=95,
  serialized_end=161,
  methods=[
  _descriptor.MethodDescriptor(
    name='GetNodeInfo',
    full_name='xrd.Base.GetNodeInfo',
    index=0,
    containing_service=None,
    input_type=_GETNODEINFOREQ,
    output_type=_GETNODEINFORESP,
    options=None,
  ),
])
_sym_db.RegisterServiceDescriptor(_BASE)

DESCRIPTOR.services_by_name['Base'] = _BASE

# @@protoc_insertion_point(module_scope)
