...
from pyxrdlib.pyxrdlib import mnemonic2bin, hstr2bin, bin2hstr

from xrd.core import config
from xrd.core.Wallet import Wallet, WalletDecryptionError
from xrd.core.misc.helper import parse_hexblob, parse_qaddress
...
from xrd.crypto.xmss import XMSS, hash_functions
from xrd.generated import xrd_pb2_grpc, xrd_pb2

ENV_xrd_WALLET_DIR = 'ENV_xrd_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return xrd_pb2_grpc.PublicAPIStub(channel)
...
@click.version_option(version=config.dev.version, prog_name='xrd Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_xrd_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def xrd(ctx, verbose, host, port_pub, wallet_dir, json):
    """
    xrd Command Line Interface
    """
...
@xrd.command()
@click.option('--src', type=str, default='', prompt=True, help='signer xrd address')
@click.option('--master', type=str, default='', prompt=True, help='master xrd address')
@click.option('--addr_to', type=str, default='', prompt=True, help='xrd Address receiving this message (optional)')
...
@xrd.command()
@click.option('--src', type=str, default='', prompt=True, help='source xrd address')
@click.option('--master', type=str, default='', prompt=True, help='master xrd address')
...
@xrd.command()
@click.option('--src', type=str, default='', prompt=True, help='signer xrd address')
@click.option('--master', type=str, default='', help='master xrd address')
...
@click.option('--owner', default='', prompt=True, help='Owner xrd address')
...
@xrd.command()
@click.option('--owner', default='', prompt=True, help='source xrd address')
@click.pass_context
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(xrd_pb2.GetNodeStateReq())
