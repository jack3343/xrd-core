...
from pyxrdlib.pyxrdlib import hstr2bin, mnemonic2bin, bin2hstr, xrdHelper

from xrd.core import config
from xrd.core.AddressState import AddressState
from xrd.daemon.helper import logger
from xrd.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from xrd.services.WalletAPIService import WalletAPIService
from xrd.generated import xrd_pb2, xrd_pb2_grpc, xrdwallet_pb2
from xrd.generated.xrdwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
pid = os.path.join(config.user.wallet_dir, 'xrd_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = xrd_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
    ...
    def to_plain_address_amount(address_amount):
        am = xrdwallet_pb2.PlainAddressAmount()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = xrd_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = xrd_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = xrd_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(xrd_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            xrd_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(xrd_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(xrd_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(xrd_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(xrd_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(xrd_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(xrd_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(xrd_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)

    def get_address_from_pk(self, pk: str) -> str:
        return self.address_to_qaddress(xrdHelper.getAddress(bytes(hstr2bin(pk))))

    def get_node_info(self):
        return self._public_stub.GetNodeState(xrd_pb2.GetNodeStateReq())
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='xrd node')
    parser.add_argument('--xrddir', '-d', dest='xrd_dir', default=config.user.xrd_dir,
    ...
    parser.add_argument('--network-type', dest='network_type', choices=['mainnet', 'testnet'],
                        default='mainnet', required=False, help="Runs xrd Testnet Node")
...
def main():
    ...
    daemon = Daemonize(app="xrd_walletd", pid=pid, action=run)
    daemon.start()
