...
from xrd.generated import xrd_pb2_grpc, xrd_pb2, xrdmining_pb2, xrdmining_pb2_grpc
...
def get_addr_state(addr: bytes) -> AddressState:
    ...
    response = stub.GetAddressState(request=xrd_pb2.GetAddressStateReq(address=addr))
...
def valid_payment_permission(public_stub, master_address_state, payment_xmss, json_slave_txn):
    ...
    if access_type == -1:
        ...
        public_stub.PushTransaction(request=xrd_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
        return None
...
def api_proxy(api_method_name):
    ...
    :return:
    """
    stub = xrd_pb2_grpc.PublicAPIStub(grpc.insecure_channel('{}:{}'.format(config.user.public_api_host,
                                                                           config.user.public_api_port),
                                                            options=[('grpc.max_receive_message_length', 10485760)]))
    public_api = xrd_pb2.DESCRIPTOR.services_by_name['PublicAPI']
    api_method = public_api.FindMethodByName(api_method_name)
    api_request = getattr(xrd_pb2, api_method.input_type.name)()
...
def getlastblockheader(height=0):
    ...
    request = xrdmining_pb2.GetLastBlockHeaderReq(height=height)
...
def getblocktemplate(reserve_size, wallet_address):
    stub = get_mining_stub()
    request = xrdmining_pb2.GetBlockToMineReq(wallet_address=wallet_address.encode())
...
def getbalance():
    stub = get_public_stub()
    grpc_response = stub.GetOptimizedAddressState(request=xrd_pb2.GetAddressStateReq(address=payment_slaves[0]))
...
def getheight():
    stub = get_public_stub()
    grpc_response = stub.GetHeight(request=xrd_pb2.GetHeightReq())
...
def submitblock(blob):
    stub = get_mining_stub()
    request = xrdmining_pb2.SubmitMinedBlockReq(blob=bytes(hstr2bin(blob)))
...
def getblockminingcompatible(height):
    stub = get_mining_stub()
    request = xrdmining_pb2.GetBlockMiningCompatibleReq(height=height)
...
def transfer(destinations, fee, mixin, unlock_time):
    ...
    response = stub.PushTransaction(request=xrd_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
...
def main():
    ...
    mining_stub = xrdmining_pb2_grpc.MiningAPIStub(grpc.insecure_channel('{0}:{1}'.format(config.user.mining_api_host,
                                                                                          config.user.mining_api_port)))
    public_stub = xrd_pb2_grpc.PublicAPIStub(grpc.insecure_channel('{0}:{1}'.format(config.user.public_api_host,
                                                                                    config.user.public_api_port),
